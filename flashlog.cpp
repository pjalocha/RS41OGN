...
template <class OGNx_Packet>
 static void Read(OGNx_Packet &Packet, uint32_t *ReadAddr)
{ uint32_t *PageAddr = getPageAddr(ReadAddr);
  Packet.HeaderWord = PageAddr[0];
  memcpy(Packet.Data, ReadAddr, 4*sizeof(uint32_t)); }

template <class OGNx_Packet>
 static bool Write(OGNx_Packet &Packet, uint32_t Time, bool NewPage=0)            // write the Packet (without Header) into the Flash log
{ int16_t Delay=0;
...
template <class OGNx_Packet>
 static bool Process(OGNx_Packet &Packet, uint32_t Time)                      // process position packet: decide whether to store it or not
{ uint32_t TimeDelta = Time-PrevTime;                                         // [sec] time since previously stored packet
  ...
  OGNx_Packet *Prev = (OGNx_Packet *)(PrevPacket-1);                          // address (tweaked) of the previously stored packet
...
template <class OGNx_Packet>
 bool FlashLog_Process(OGNx_Packet &Packet, uint32_t Time)                      // process position packet: decide whether to store it or not
...
template
 bool FlashLog_Process<OGN_Packet>(OGN_Packet &Packet, uint32_t Time);
...
{ uint32_t First=Page[2]; uint32_t Last=Page[3];
  ...
  if(Packet->Header.Relay) return 0;
