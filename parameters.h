...
/*
  ...
  void ErasePage(uint32_t *Addr) const // erase a 4x64 = 256-byte page
  { NVMCTRL->ADDR.reg = ((uint32_t)Addr)>>1;
    NVMCTRL->CTRLA.reg = NVMCTRL_CTRLA_CMDEX_KEY | NVMCTRL_CTRLA_CMD_ER;
    while (!NVMCTRL->INTFLAG.bit.READY) { }
  }

  void EraseMultiPage(uint32_t *Addr, uint32_t Size=1024) const
  { for( ; ; )
    { ErasePage(Addr);
      Addr+=0x40; Size-=0x100;
      if(Size==0) break; }
  }

  // void WritePage(uint32_t *Addr, uint32_t *Src, uint32_t Size) const
  // {
  // }
